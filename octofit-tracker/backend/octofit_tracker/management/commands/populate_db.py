...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        Activity.objects.create(user=users[3], type='Martial Arts', duration=90, date=timezone.now().date())

        # Create workouts
        w1 = Workout.objects.create(name='Super Strength', description='Strength workout for heroes')
        w2 = Workout.objects.create(name='Agility Training', description='Agility and reflexes')
